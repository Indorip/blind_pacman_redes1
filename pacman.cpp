...
int Pacman::updatePacman(Grid* grid, DirectionType directionPacman) {
    ...
    if (CHECK_BOUNDS(nextPos.x, nextPos.y)) return -2;
...
void GameState::printGridBlind() {
    for (int i = this->pacman.position.y - this->pacman.visibility;
